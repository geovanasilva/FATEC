					...
					if( !empty( $email ) && filter_var( $email, FILTER_VALIDATE_EMAIL ) && strlen( $email ) <= 50 )
					{
						if( !empty( $password ) && strlen( $password ) <= 50 )
						{
							...
							if( $user->Login( $email, $password ) == 1 )
							{
								...
								if( $access == 'Admin') $headerLoc .= 'panels-admin.php';
								else if( $access == 'Usuario') $headerLoc .= 'index-user.php';
							}
							else
								$headerLoc .= 'panels-admin.php?email='.$email;
		...
		else
		{
			...
			{				
				...
				if( !empty($senhaAtual) && $senhaAtual == $originValues[1]['PW_Usuario'] )
				{
                    ...
                    if( !empty($novaSenha) && !empty($confirm) && $novaSenha == $confirm && $novaSenha != $originValues[1]['PW_Usuario'] && strlen($novaSenha) <= 50 )
									...
									<div class="col-md-12">
										<form role="form">
											...
											<div class="form-group text-center">
												<button class="btn btn-danger" type="submit" title="Excluir">Excluir</button>
											</div>
										</form>
									</div>
								</div>
