...
<body>
	<?php
		...
		{			
			...
			if( $user->Login( $username, $password ) == 1 )
			{
				...
				if( $access == 'Admin') $headerLoc .= 'http://backend.dev.fatecpg.com/index-admin.php';
				else if( $access == 'Usuario') $headerLoc .= 'http://backend.dev.fatecpg.com/index-user.php';
			}
			else
				$headerLoc .= 'http://backend.dev.fatecpg.com/Index.php';
		...
		if( isset( $_POST['register-submit'] ) )
		{
			...
			if( !empty( $username ) && !empty( $password ) && !empty( $confirm ) && !empty( $email ) )
			{
				...
				if( $user->CadastrarUsuario( $username, $password, $acesso, $email ) == 1 )
					$headerLoc .= 'http://backend.dev.fatecpg.com/index-user.html';
				else
					$headerLoc .= 'http://backend.dev.fatecpg.com/Index.php?username='.$username.'&email='.$email.'&sent=enviado';
	...
	<script src="assets/js/datepicker-pt-BR.js">
		< script >
